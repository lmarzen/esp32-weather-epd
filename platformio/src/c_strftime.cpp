 ...
 * size max.
 */
size_t c_strftime(char *s, size_t maxsize, const char *format, const struct tm *timeptr)
{
  char *endp = s + maxsize;
  char *start = s;
  char tbuf[100];
  ...
  again:
    ...
    case 'c':
      c_strftime(tbuf, sizeof(tbuf), LC_D_T_FMT, timeptr);
      break;
    ...
    case 'D': // date as %m/%d/%y
      c_strftime(tbuf, sizeof(tbuf), "%m/%d/%y", timeptr);
      break;
    ...
    case 'F': // ISO 8601 date representation
    {
      ...
      char m_d[10];
      c_strftime(m_d, sizeof(m_d), "-%m-%d", timeptr);
      size_t min_fw = 10;
      ...
      strcat(tbuf, m_d);
#else
      c_strftime(tbuf, sizeof(tbuf), "%Y-%m-%d", timeptr);
#endif // POSIX_2008
    }
    break;
    ...
    case 'r': // time in a.m. or p.m. notation
      c_strftime(tbuf, sizeof(tbuf), LC_T_FMT_AMPM, timeptr);
      break;

    case 'R': // time as %H:%M
      c_strftime(tbuf, sizeof(tbuf), "%H:%M", timeptr);
      break;
    ...
    case 'x': // appropriate date representation
      c_strftime(tbuf, sizeof(tbuf), LC_D_FMT, timeptr);
      break;

    case 'X': // appropriate time representation
      c_strftime(tbuf, sizeof(tbuf), LC_T_FMT, timeptr);
      break;
