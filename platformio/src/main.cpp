...
void beginDeepSleep(unsigned long &startTime, tm *timeInfo) {
  ...
  if (timeInfo->tm_min >= 58) { // if we are within 2 minutes of the next hour,
                                // then round up for the
    // purposes of bed time
    curHour = (curHour + 1) % 24;
    extraHoursUntilWake += 1;
  }

  if (BED_TIME < WAKE_TIME && curHour >= BED_TIME &&
      curHour < WAKE_TIME) { // 0              B   v  W  24
    // |--------------zzzzZzz---|
    extraHoursUntilWake += WAKE_TIME - curHour;
  } else if (BED_TIME > WAKE_TIME && curHour < WAKE_TIME) { // 0 v W B    24
    // |zZz----------------zzzzz|
    extraHoursUntilWake += WAKE_TIME - curHour;
  } else if (BED_TIME > WAKE_TIME && curHour >= BED_TIME) { // 0   W B  v 24
    // |zzz----------------zzzZz|
  ...
  if (extraHoursUntilWake ==
      0) { // align wake time to nearest multiple of SLEEP_DURATION
    sleepDuration =
        SLEEP_DURATION * 60ULL -
        ((timeInfo->tm_min % SLEEP_DURATION) * 60ULL + timeInfo->tm_sec);
  } else { // align wake time to the hour
  ...
  Serial.println("Awake for " + String((millis() - startTime) / 1000.0, 3) +
                 "s");
...
void setup() {
  ...
  if (batteryVoltage <= LOW_BATTERY_VOLTAGE) {
    ...
    if (batteryVoltage <= CRIT_LOW_BATTERY_VOLTAGE) { // critically low battery
      ...
      Serial.println("Deep-sleep for " + String(LOW_BATTERY_SLEEP_INTERVAL) +
                     "min");
    }
    esp_deep_sleep_start();
  }
  ...
  if (digitalRead(PIN_CONFIGURE_WIFI) == LOW) {
    ...
    initDisplay();
    do {
      drawError(wifi_x_196x196, "Weather Station is in",
  ...
  if (!timeConfigured) { // Failed To Fetch The Time
